...
BitcoinExchange::BitcoinExchange(str db_file): database()
{
	if (!validFileExtension(db_file, ".csv"))
		throw std::runtime_error("BTC -> Invalid file extension! Expected extension (.csv)");

}

// BitcoinExchange::BitcoinExchange(str db_file, str inputfile): database()
// {
// 	if (!validFileExtension(db_file, ".csv"))
// 		throw std::runtime_error("BTC -> Invalid file extension! Expected extension (.csv)");
// 	if (!validFileExtension(inputfile, ".txt"))
// 		throw std::runtime_error("BTC -> Invalid file extension! Expected extension (.txt)");
...
void BitcoinExchange::parseInput(str inputfile)
{
	...
	if (database.empty())
		throw std::runtime_error("BTC -> No Database is loaded in this object!");
}

void BitcoinExchange::parseDatabase(str db_file)
{
	if (!validFileExtension(db_file, ".csv"))
		throw std::runtime_error("BTC -> Invalid file extension (" + db_file + ")! Expected extension (.csv)");
